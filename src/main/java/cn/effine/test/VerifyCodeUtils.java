...
public class VerifyCodeUtils {
	...
	public static void outputImage(int w, int h, OutputStream os, String code)
		...
		grap.setColor(Color.WHITE);		// 设置边框色
		grap.setFont(new Font("黑体" , Font.ITALIC , 30));	// 字体
		grap.fillRect(0, 0, w, h);

		Color color = getRandColor(200, 250);
		//grap.setColor(c);// 设置背景色
		//grap.fillRect(0, 2, w, h - 4);

		// 绘制干扰线
		/*
		Random random = new Random();
		grap.setColor(getRandColor(160, 200));// 设置线条的颜色
		for (int i = 0; i < 20; i++) {
		...
		*/
		...
		shear(grap, w, h, color);// 使图片扭曲

		grap.setColor(getRandColor(100, 160));
		int fontSize = h - 4;
		...
		for (int i = 0; i < verifySize; i++) {
			...
			affine.setToRotation(
					Math.PI / 4 * rand.nextDouble()
							...
							* i + fontSize / 2, h / 2);
			grap.setTransform(affine);
			grap.drawChars(chars, i, 1, ((w - 10) / verifySize) * i + 5, h / 2
					+ fontSize / 2 - 10);
		}
	...
	public static void main(String[] args) throws IOException {
		File dir = new File("C:/home/data/picture/verify");
		int w = 100, h = 42;
